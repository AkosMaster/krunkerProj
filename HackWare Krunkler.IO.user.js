...
function menuOverride () {
    ...
    var newmenu = "HackWare Settings<br><div class='settName'>Aimbot <label class='switch'><input type='checkbox' onclick='enablehack(0,this.checked)' " + ar[0] + "><span class='slider'></span></label></div> \
...
<div class='settName'> Extra Player Info <label class='switch'><input type='checkbox' onclick='enablehack(2,this.checked)' " + ar[2] + "><span class='slider'></span></label></div> \
<div class='settName'> Bhop <label class='switch'><input type='checkbox' onclick='enablehack(3,this.checked)' " + ar[3] + "><span class='slider'></span></label></div><br>";
    setTimeout(function() {
...
function setupMenu() {
setTimeout(function() {
    document.getElementsByClassName("menuLink")[8].setAttribute( "id","smenu");
    document.getElementsByClassName("menuLink")[8].innerHTML="Cheat Settings";
    document.getElementsByClassName("menuLink")[8].setAttribute( "onClick","showWindow(1); menuOverride();");

    document.getElementsByClassName("menuLink")[8].setAttribute( "data-ops",'["checked","checked", "checked","checked"]');
...
GM_xmlhttpRequest({
    method: "GET",
    url: `https://cdn.jsdelivr.net/gh/AkosMaster/krunkerProj@098840d/gamev15.js`,
    onload: jsresp => {
        ...
        GM_xmlhttpRequest({
            ...
            onload: inRes => {
                ...
                newBody = dbody.replace(/<script src="js\/game\.js\?build=.+"><\/script>/g, `<script src="https://cdn.jsdelivr.net/gh/AkosMaster/krunkerProj@098840d/gamev15.js"></script><script type="text/plain" src="js/game.js?build=fL02f"></script>`);
                document.open();
...
unsafeWindow.mdlsettings = {}
  ...
  function handleMessage(m){
    ...
    unsafeWindow.mnxrecoil = (me, inputs) => {
        ...
        for (let playerInfo of playerInfos.children){
                ...
                let rectCode='<h4 style="text-align: center;"><div style="width:' + 1000/(Math.round(dist3(playerObj, me))/5)*($(window).width()/1920) + 'px;height:' + 1000/(Math.round(dist3(playerObj, me))/7)*($(window).height()/1080) + 'px;border:3px solid #000; display: inline-block;"></h4>'
                pname.innerHTML = `${playerObj.name} (${Math.round(dist3(playerObj, me))/10} mm)<h4 id="` + playerObj.name + `" style="position: absolute; left: 0px; top: 0px; color: white; text-align: center; margin-top: 20px; margin-bottom: 0px;">[${playerObj.weapon.name}]<br><br><br></h4> ` + rectCode + ``;
  ...
  function addListener(socket){
    ...
    krSocket = socket;
    $('#aHolder').css({opacity: 0, cursor: "default", marginTop: 5000, position: "absolute"});
    //unsafeWindow.Ze("HackWare", `Welcome to Krunker.IO!`);
